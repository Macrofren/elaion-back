...
async def lifespan(app: FastAPI):
    ...
    As alterações de schema NÃO são aplicadas em runtime: elas pertencem
    exclusivamente às migrações Alembic (`alembic upgrade head`). Isso evita
    drift de schema, condições de corrida entre instâncias e a manipulação
    manual da tabela `alembic_version`.
    """
    yield
